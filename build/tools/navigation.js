...
export function registerNavigationTools(server) {
    ...
    server.tool("browser_launch", "启动Chrome浏览器实例，支持headless模式和代理设置", {
    ...
    // browser_close - 关闭浏览器
    server.tool("browser_close", "关闭浏览器实例，释放资源", {}, async () => {
        try {
    ...
    server.tool("page_navigate", "导航到指定URL，等待页面加载完成", {
    ...
    server.tool("page_screenshot", "截取当前页面截图，返回base64编码图片", {
    ...
    server.tool("page_get_content", "获取页面HTML内容或指定元素的内容", {
