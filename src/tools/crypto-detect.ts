...
export function registerCryptoDetectTools(server: McpServer) {
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        const detections: Array<{ pattern: string; algorithm: string; confidence: number; location: string; matchedCode: string }> = [];

        // 启用Debugger获取脚本
        try { await cdp.send("Debugger.enable"); } catch { /* already enabled */ }

        // 收集所有脚本源码
        const scripts: Array<{ id: string; url: string; source: string }> = [];

        // 从已有缓存 + 重新获取
        const scriptsParsed: any[] = [];
        const handler = (event: any) => { scriptsParsed.push(event); };
        cdp.on("Debugger.scriptParsed", handler);
        await new Promise((resolve) => setTimeout(resolve, 500));
        cdp.off("Debugger.scriptParsed", handler);

        for (const script of scriptsParsed) {
          if (script.url?.includes("extension://") || script.url?.includes("devtools://")) continue;
          try {
        ...
        const runtimeCheck = await page.evaluate(() => {
          const found: string[] = [];
          if ((window as any).CryptoJS) found.push("CryptoJS (全局对象存在)");
          if ((window as any).JSEncrypt) found.push("JSEncrypt (全局对象存在)");
          if ((window as any).forge) found.push("node-forge (全局对象存在)");
          if ((window as any).sm2) found.push("sm2国密 (全局对象存在)");
          if ((window as any).sm4) found.push("sm4国密 (全局对象存在)");
          if ((window as any).md5) found.push("md5函数 (全局对象存在)");
          if ((window as any).SHA256) found.push("SHA256函数 (全局对象存在)");
          if ((window as any).RSAUtils) found.push("RSAUtils (全局对象存在)");
          return found;
        });
  ...
  server.tool(
    ...
    async ({ paramName, sampleValues }) => {
      try {
          ...
          for (const req of loginRequests) {
            if (req.postData) {
              try {
                ...
                if (json[paramName]) samples.push(json[paramName]);
              } catch {
        ...
        const analysis: string[] = [`=== 参数 "${paramName}" 加密分析 ===\n`];
    ...
    async () => {
      try {
        ...
        const libs = await page.evaluate(() => {
          const detected: Array<{ name: string; version: string; methods: string[] }> = [];

          // CryptoJS
          if ((window as any).CryptoJS) {
            const cjs = (window as any).CryptoJS;
          ...
          if ((window as any).JSEncrypt) {
            detected.push({
              name: "JSEncrypt",
              version: (window as any).JSEncrypt.version || "unknown",
          ...
          if ((window as any).forge) {
            const f = (window as any).forge;
            const methods = Object.keys(f).slice(0, 15);
            detected.push({ name: "node-forge", version: f.version || "unknown", methods });
          }

          // sm-crypto
          if ((window as any).sm2) {
            detected.push({
              name: "sm-crypto (SM2)",
              version: "unknown",
              methods: Object.keys((window as any).sm2).slice(0, 10),
            });
          }
          if ((window as any).sm4) {
            detected.push({
              name: "sm-crypto (SM4)",
              version: "unknown",
              methods: Object.keys((window as any).sm4).slice(0, 10),
            });
          }
  ...
  server.tool(
    ...
    async ({ algorithm }) => {
      try {
        const page = browserManager.getPage();
        const cdp = browserManager.getCDPSession();
        const findings: string[] = ["=== 密钥/参数提取结果 ===\n"];

        // 1. 从Runtime获取
        const runtimeKeys = await page.evaluate((algo: string | undefined) => {
          const keys: Array<{ name: string; value: string; source: string }> = [];
    ...
    async ({ algorithm, plaintext, expected, key, iv, encoding }) => {
      try {
        ...
        switch (algorithm) {
          ...
          case "AES-CBC":
          case "AES-ECB": {
            if (!key) return { content: [{ type: "text", text: "AES加密需要提供key参数" }], isError: true };
          ...
          case "DES": {
            if (!key) return { content: [{ type: "text", text: "DES加密需要提供key参数" }], isError: true };
            const keyParsed = CryptoJS.enc.Utf8.parse(key);
            const options: any = { mode: CryptoJS.mode.ECB, padding: CryptoJS.pad.Pkcs7 };
            if (iv) options.iv = CryptoJS.enc.Utf8.parse(iv);
            const encrypted = CryptoJS.DES.encrypt(plaintext, keyParsed, options);
            result = outputEncoding === "hex" ? encrypted.ciphertext.toString() : encrypted.toString();
            break;
          }
          case "3DES": {
            if (!key) return { content: [{ type: "text", text: "3DES加密需要提供key参数" }], isError: true };
            const keyParsed = CryptoJS.enc.Utf8.parse(key);
