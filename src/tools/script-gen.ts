...
import { generatePythonDecryptScript, generatePythonEncryptScript, generatePythonBruteScript, ScriptParams } from "../generators/python-template.js";
...
export function registerScriptGenTools(server: McpServer) {
  ...
  server.tool(
    ...
    async ({ language, algorithm, key, iv, mode, padding, encoding, publicKey, testPlaintext, testCiphertext }) => {
      try {
        ensureOutputDir();
        const params: ScriptParams = { algorithm, key, iv, mode, padding, encoding, publicKey, testPlaintext, testCiphertext };
  ...
  server.tool(
    ...
    async ({ language, algorithm, loginUrl, paramName, key, iv, mode, encoding, publicKey }) => {
      try {
        ensureOutputDir();
        const params: ScriptParams = { algorithm, key, iv, mode, encoding, publicKey, loginUrl, paramName };
