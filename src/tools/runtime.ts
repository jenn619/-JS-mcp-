  ...
  server.tool(
    ...
    async ({ functionName, args }) => {
      try {
        ...
        const result = await page.evaluate(async (funcName: string, argsJson: string) => {
          try {
            const fn = (window as any)[funcName] || eval(funcName);
    ...
    async ({ pattern }) => {
      try {
        ...
        const result = await page.evaluate((pat: string) => {
          ...
          const found: Array<{ name: string; type: string; preview: string }> = [];
  ...
  server.tool(
    ...
    async ({ functionName }) => {
      try {
        ...
        // 在页面中注入Hook
        const success = await page.evaluate((funcName: string) => {
          try {
            const parts = funcName.split(".");
            let obj: any = window;
            for (let i = 0; i < parts.length - 1; i++) {
              obj = obj[parts[i]];
              if (!obj) return { success: false, error: `对象 ${parts.slice(0, i + 1).join(".")} 不存在` };
            }
            ...
            // 创建全局Hook日志
            if (!(window as any).__hookLogs) {
              (window as any).__hookLogs = [];
            }

            obj[lastPart] = function (...args: any[]) {
              ...
              (window as any).__hookLogs.push(logEntry);
              return result;
            };
  ...
  server.tool(
    ...
    async ({ functionName, clear }) => {
      try {
        ...
        const logs = await page.evaluate((filterName: string | undefined, shouldClear: boolean) => {
          const allLogs = (window as any).__hookLogs || [];
          let filtered = filterName
            ? allLogs.filter((l: any) => l.functionName === filterName)
            : allLogs;
        ...
        const output = logs.map((log: any, i: number) => {
          return [
