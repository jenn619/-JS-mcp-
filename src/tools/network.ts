...
import { requestStore, RequestRecord } from "../storage/request-store.js";
    ...
    async ({ urlPatterns }) => {
      try {
        ...
        const patterns = urlPatterns
          ? urlPatterns.map((p) => ({ urlPattern: p, requestStage: "Response" as const }))
          : [{ urlPattern: "*", requestStage: "Response" as const }];
    ...
    async ({ method, urlPattern, loginOnly }) => {
      try {
        ...
        const summary = records.map((r) => {
          const flags: string[] = [];
          if (r.isLoginRequest) flags.push("🔐登录请求");
          if (r.encryptedParams.length > 0) flags.push(`🔒加密参数: ${r.encryptedParams.join(",")}`);
...
function parseHeadersArray(headers: Array<{ name: string; value: string }>): Record<string, string> {
