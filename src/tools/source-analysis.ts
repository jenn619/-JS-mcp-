...
// 脚本缓存
const scriptsCache: Map<string, ScriptInfo> = new Map();
const sourceCache: Map<string, string> = new Map();

export function registerSourceAnalysisTools(server: McpServer) {
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        // 启用Debugger获取脚本
        const scriptsParsed: ScriptInfo[] = [];

        const scriptHandler = (event: any) => {
  ...
  server.tool(
    ...
    async ({ keyword, isRegex, contextLines }) => {
      try {
        ...
        // 使用CDP搜索
        const searchResult = await cdp.send("Debugger.searchInContent" as any, {
        ...
        for (const [scriptId, info] of scriptsCache.entries()) {
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            if (regex) regex.lastIndex = 0; // reset for next test
...
function extractFunctionBody(source: string, startIndex: number): string | null {
...
function findFunctionInSource(source: string, funcName: string): string | null {
  const patterns = [
...
function extractVariableReferences(funcBody: string, fullSource: string): string[] {
  const refs: string[] = [];
...
function extractInnerFunctionCalls(funcBody: string): string[] {
  const calls: string[] = [];
